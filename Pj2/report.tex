...
\title{Digital Logic Design: 8-bit Adders Report}
...
The foundation of our adders is the 1-bit Full Adder module (\texttt{full\_adder}). It takes two bits $A, B$ and a carry-in $C_{in}$ as inputs. We assigned a delay of $t_{sum} = 10$\,ns for the sum output and $t_{co} = 8$\,ns for the carry-out ($C_o$). These values were chosen arbitrarily but reflect common hardware characteristics where carry logic is prioritized.
...
A critical part of this project was analyzing how different architectures affect performance. We used the following delay parameters: $t_{sum} = 10$\,ns, $t_{co} = 8$\,ns, and $t_{mux} = 5$\,ns.
...
\[ \text{Delay}_{RCA} = 7 \times 8\,\text{ns} + 10\,\text{ns} = 66\,\text{ns} \]
The final carry-out is ready at $8 \times 8\,\text{ns} = 64$\,ns.
...
\[ \text{Delay}_{CSA} = (4 \times 8\,\text{ns}) + 5\,\text{ns} = 37\,\text{ns} \text{ (for Cout)} \]
The upper sum bits are ready slightly later at $34\,\text{ns} + 5\,\text{ns} = 39$\,ns.

\paragraph{Final Verdict:}
The Carry Select Adder achieves a significant performance boost. By "spending" more hardware to pre-calculate potential outcomes, we reduced the total critical path delay from \textbf{66\,ns} to \textbf{39\,ns} (a ~40\% improvement).
...
Critical Path (Worst Case) & 66\,ns & 39\,ns \\ \hline
